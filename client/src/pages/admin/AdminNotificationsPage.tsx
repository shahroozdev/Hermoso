...
const AdminNotificationsPage = () => {
  ...
  const [recipientsNotif, setRecipientsNotif] =
    ...
    try {
      ...
      const rows = [
        ...
        ...items.map((item) => [
          ...
          item.createdAt
            ? `${formatDateInput(item.createdAt)} ${formatTimeAMPM(item.createdAt)}`
            : "",
      ...
      downloadCsv(
        `hermoso-sent-notifications-${new Date().toISOString().slice(0, 10)}.csv`,
        rows,
      );
      showToast("Sent notifications report downloaded.");
    } catch (err) {
      showToast(
        err.response?.data?.message || "Failed to export sent notifications",
        "error",
      );
    } finally {
  ...
  return (
    <>
      <div className="ha-notifications-page">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">Notifications</h2>
          <div className="ha-page-actions">
            <button className="ha-act-btn" onClick={handleMarkAllRead}>
              Mark All Read
            </button>
            <button
              className="ha-act-btn"
              onClick={handleExportSentSummary}
...
const RecipientReportModal = ({
  ...
  const filteredRecipients = useMemo(() => {
    ...
    return recipients.filter((r) => {
      if (
        term &&
        !(r.user?.name || "Deleted user").toLowerCase().includes(term)
      )
        return false;
        ...
        <>
          <div style={{ flexShrink: 0 }}>
            ...
            <p className="mb-3 text-sm text-muted">
              {recipients.length.toLocaleString()} recipient
              {recipients.length === 1 ? "" : "s"} &middot;{" "}
