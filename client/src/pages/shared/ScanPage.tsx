...
const ScanPage = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      currentStep++;
      const progress = Math.min(100, Math.round((currentStep / SCAN_STEPS.length) * 100));
  ...
  const analyzeImage = async (file: File) => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } }).response?.data?.message || 'Analysis failed. Try a clearer photo.';
      setError(msg);
    } finally {
      setAnalyzing(false);
    }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      <div className="shell-panel rounded-2xl p-6">
        ...
        <p className="mt-1 text-sm text-muted">Upload a selfie for professional AI-powered skin analysis</p>
      </div>
      ...
      {!preview && (
        <div className="shell-panel rounded-2xl p-8 text-center">
          ...
          <p className="mb-2 text-sm text-muted">Take a well-lit, front-facing selfie for best results</p>
      ...
      {preview && analyzing && (
        <div className="shell-panel rounded-2xl overflow-hidden">
          <div className="relative">
            <img src={preview} alt="Uploaded" className="w-full max-h-96 object-contain bg-black/5" />
          ...
          <div className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-[var(--accent-2)]">{scanProgress}%</span>
            </div>
                ...
                return (
                  ...
                  >
                    {isDone ? (
                      ...
                      <svg className="h-4 w-4 shrink-0" viewBox="0 0 16 16" fill="currentColor" opacity="0.4">
        ...
        <div className="rounded-2xl border border-rose-400/40 bg-rose-500/10 p-4 text-sm text-rose-600">
          ...
          {showGuidance && result?.faceGuidance?.length ? (
            <ul className="mt-2 list-inside list-disc space-y-1 text-xs">
              {result.faceGuidance.map((g, i) => <li key={i}>{g}</li>)}
            </ul>
          ) : null}
          <button onClick={reset} className="mt-3 text-xs font-semibold underline underline-offset-2">
            Try Again
          </button>
        </div>
      )}
          ...
          <div className="shell-panel rounded-2xl p-6">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center sm:text-left">
                <h3 className="text-lg font-semibold">Your Skin Health Score</h3>
          ...
          <SectionCard title="Skin Tone & Tanning">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="text-sm font-semibold capitalize">{result.skinTone.tone}</p>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-purple-500" style={{ width: `${result.skinTone.evenness}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.skinTone.evenness}%</span>
                </div>
              </div>
            </div>
            <div>
            ...
            <SectionCard title="Eyebrow Assessment">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="text-sm font-semibold capitalize">{result.eyebrows.archShape}</p>
                </div>
                <div>
                  <p className="text-xs text-muted">Fullness</p>
                  <p className="text-sm font-semibold">{result.eyebrows.fullness}/5</p>
                </div>
                <div>
                  <p className="text-xs text-muted">Symmetry</p>
                  <p className="text-sm font-semibold">{result.eyebrows.leftRightSymmetry}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted">Tail Length</p>
                  <p className="text-sm font-semibold capitalize">{result.eyebrows.tailLength}</p>
                </div>
              </div>
          ...
          <SectionCard title="Hydration & Texture">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-sky-500" style={{ width: `${result.hydration.hydrationPercent}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.hydration.hydrationPercent}%</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-emerald-500" style={{ width: `${result.hydration.textureRating}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.hydration.textureRating}%</span>
                </div>
              </div>
            </div>
            {result.hydration.dehydrationZones?.length ? (
              <div>
                ...
                <p className="mt-1 text-sm capitalize">{result.hydration.dehydrationZones.join(', ')}</p>
              </div>
            ) : null}
          ...
          <SectionCard title="Dark Circles & Under-Eye">
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <p className="text-sm font-semibold" style={{ color: DARK_CIRCLE_COLORS[result.darkCircles.type] }}>
                  Type {result.darkCircles.type} — {DARK_CIRCLE_LABELS[result.darkCircles.type]}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted">Severity</p>
                <p className="text-sm font-semibold capitalize">{result.darkCircles.severity}</p>
              </div>
              <div>
                <p className="text-xs text-muted">Color Delta</p>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${result.darkCircles.colorDelta}%`, backgroundColor: DARK_CIRCLE_COLORS[result.darkCircles.type] }} />
                  </div>
                  <span className="text-sm font-semibold">{Math.round(result.darkCircles.colorDelta)}%</span>
                </div>
              </div>
            </div>
          ...
          <SectionCard title="Acne & Breakout Zones">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-rose-500" style={{ width: `${result.acne.overallSeverity}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.acne.overallSeverity}%</span>
                </div>
              </div>
              <div>
                <p className="text-xs text-muted">Affected Zones</p>
                <p className="text-sm capitalize">
                  {result.acne.zones?.filter((z) => z.type !== 'none').map((z) => z.area).join(', ') || 'None detected'}
                </p>
              </div>
            </div>
            {result.acne.zones?.filter((z) => z.type !== 'none').length ? (
              <div className="grid gap-2 sm:grid-cols-3">
                {result.acne.zones.filter((z) => z.type !== 'none').map((zone) => (
                  <div key={zone.area} className="rounded-lg border border-[var(--border)] p-3">
                    ...
                    <span
                      ...
                      style={{ backgroundColor: `${ACNE_TYPE_COLORS[zone.type]}15`, color: ACNE_TYPE_COLORS[zone.type] }}
                    >
                      {zone.type}
                    </span>
                  </div>
          ...
          <SectionCard title="Lip Pigmentation">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="text-sm font-semibold capitalize">{result.lipPigmentation.darknessLevel}</p>
              </div>
              <div>
                <p className="text-xs text-muted">Melanin Index</p>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-pink-600" style={{ width: `${result.lipPigmentation.melaninIndex}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.lipPigmentation.melaninIndex}%</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-orange-500" style={{ width: `${result.lipPigmentation.unevenness}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.lipPigmentation.unevenness}%</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-[var(--surface-soft)] overflow-hidden">
                    <div className="h-full rounded-full bg-amber-500" style={{ width: `${result.lipPigmentation.drynessLevel}%` }} />
                  </div>
                  <span className="text-sm font-semibold">{result.lipPigmentation.drynessLevel}%</span>
                </div>
              </div>
            </div>
          ...
          {result.treatmentPlan?.length ? (
            <SectionCard title="AI Treatment Priority Plan">
              <div className="space-y-3">
                {result.treatmentPlan.sort((a, b) => a.priority - b.priority).map((item) => (
                  <div
                    key={item.priority}
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold text-white ${
                            item.priority === 1 ? 'bg-rose-500' : item.priority === 2 ? 'bg-amber-500' : 'bg-emerald-500'
                          }`}
                        >
                          {item.priority}
                        </span>
          ...
          {result.dietPlan ? (
            <SectionCard title="Diet & Nutrition Plan">
              ...
              <div className="mt-4 rounded-lg bg-[var(--surface-soft)] p-3">
                ...
                <p className="text-sm font-semibold">{result.dietPlan.dailyWaterIntake}</p>
              </div>
            </SectionCard>
          ) : null}
            ...
            <SectionCard title="Recommended Services">
              <div className="grid gap-3 sm:grid-cols-2">
                {result.recommendedServices.map((svc) => (
                  <div key={svc._id} className="rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-4">
                    <p className="font-semibold text-sm">{svc.name}</p>
                    <p className="mt-1 text-xs text-muted">PKR {svc.price} &middot; {svc.duration} min</p>
                  </div>
                ))}
              </div>
            </SectionCard>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              <svg className="h-4 w-4" viewBox="0 0 16 16" fill="currentColor">
                ...
                <path d="M5.82 9.16l4.36 2.68M10.18 4.16l-4.36 2.68" stroke="currentColor" strokeWidth="1.5" fill="none" />
              </svg>
              Share via WhatsApp
            </button>
            <button
            ...
            >
              <svg className="h-4 w-4" viewBox="0 0 16 16" fill="currentColor">
                <path fillRule="evenodd" d="M8 3a5 5 0 00-4.33 2.5.5.5 0 11-.87-.5A6 6 0 1114 10a.5.5 0 11-1 0 5 5 0 00-5-5z" clipRule="evenodd" />
              </svg>
              Re-scan
            </button>
            <a
