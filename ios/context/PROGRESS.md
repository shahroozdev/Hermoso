...
This whole app was written on Windows with no Swift compiler available, so **nothing here has been compiled yet**. `.github/workflows/ios-build.yml` (repo root) runs on a `macos-15` GitHub Actions runner on every push/PR touching `ios/`: `xcodegen generate` → unsigned `xcodebuild` for the iOS Simulator. It only proves the app *compiles* — it doesn't launch the simulator, exercise the UI, or touch the camera/location code paths, so real on-device testing is still needed for the Scan liveness feature especially.

---

## Phase 1: Project Setup & Foundation

- [~] Initialize Xcode project with SwiftUI — `project.yml` (XcodeGen manifest) is ready; the actual `.xcodeproj` is generated by running `xcodegen generate` from `ios/`, which requires Xcode/macOS (not available in this Windows session). See note below.
...
> **Note on building/running:** this project was scaffolded on Windows, which cannot run Xcode or the iOS Simulator. Nothing here has been compiled yet. Once opened on a Mac: `brew install xcodegen` (if needed), then `xcodegen generate` from the `ios/` directory to produce `Hermoso.xcodeproj`, then open and build. Expect to fix minor compile issues on first build — this code was written carefully but not machine-verified. The "Hermoso" wordmark also needs the actual Cormorant Garamond (Light) font file added under `Hermoso/Resources/Fonts/` to match `UIAppFonts` in `project.yml` — until then it silently falls back to the system font.
...
- [~] SummaryRow (key-value row) — only inlined per-screen so far (e.g. BookingView's `summaryRow`), not extracted as a shared component
...
- [~] ScoreRing (animated circular progress) — built inline in `ScanResultsView.overallScoreCard`, not extracted as a standalone reusable component; not yet animated on appear (Android animates the score fill over 1000ms)
- [~] SectionHeader (title + "See all" link) — only inlined in HomeView so far
...
- [~] Error + Retry view — every screen has its own inline error text + retry button; not extracted as a shared component
- [~] Empty state view — same as above, inlined per-screen (e.g. BookingsListView's "No bookings yet.")
...
- [ ] Test on real device (camera, location)
