...
// Hermoso App (customer) — sign-up always registers as "customer". After
// login the role guard rejects any non-customer account and shows a message
// directing the user to download Hermoso Business instead.
@MainActor
final class AuthViewModel: ObservableObject {
    ...
    private func performLogin() async throws {
        ...
        if let role = response.user?.role, role != UserRole.customer.rawValue {
            errorMessage = "This is a business account — download Hermoso Business to sign in."
            return
        }
    ...
    private func performRegister() async throws {
        let response = try await api.register(RegisterRequest(name: name, email: email, phone: phone, password: password, role: UserRole.customer.rawValue))
