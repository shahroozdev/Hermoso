...
fun OwnerInsightsScreen() {
    ...
    Column(modifier = Modifier.fillMaxSize().background(OwnerNavy).padding(16.dp)) {
        Card(colors = CardDefaults.cardColors(containerColor = OwnerNavyCard), shape = RoundedCornerShape(14.dp)) {
            Column(Modifier.padding(14.dp)) {
                Text("AI Business Insights", color = OwnerGoldLight, fontWeight = FontWeight.Bold)
                Text(message, color = Color(0xFF6B84A0), modifier = Modifier.padding(top = 8.dp))
            }
        }
    }
}
