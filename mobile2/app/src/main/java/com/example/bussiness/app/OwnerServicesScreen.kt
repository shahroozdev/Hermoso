        ...
        items(services) { s ->
            Card(
                ...
                Row(Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                    Column {
                        Text(s.name ?: "Service", color = OwnerTextLight, fontWeight = FontWeight.Bold)
                        Text("${s.duration ?: 0} min", color = Color(0xFF6B84A0))
                    }
                    Text("PKR ${(s.price ?: 0.0).toInt()}", color = OwnerGoldLight, fontWeight = FontWeight.Bold)
                }
            }
        }
    }
